...
class Memory(object):
    ...
    #
    def pcbOutgoingNowInDisk(self, pcbId, page):
...
#         self.memoryBlocks[numberOfBlock].addInstruction(instruction)
    #
    def pcbIncomingNowInMemory(self, pcb, page):
    ...
    def loadInstructionsToMemory(self, pcb, page):
        ...
        # Me traigo el idPcb y seteo que las instrucciones de la pagina correspondiente al bloque estan en disco
#         idPcbOutgoing = self.pcbInTable(pcb.getBlockOfPage(page))
#         self.pcbOutgoingNowInDisk(idPcbOutgoing,page)
    ...
    def assingBlockToInstructions(self,pcb):
        ...
        if(self.spaceFreeInMemory()):
            ...
            block = random.randrange(self.memoryBlocks.__len__()-1)
            instBlock = program.getPage(pcb.getNextPage()).instructions
            pcb.nextPageToLoad()
            #actualizo PCB saliente
            #idPcbOutgoing = self.pcbInTable(block)
#             print(self.lastIdPcbAssing,block)
#             self.pcbOutgoingNowInDisk(self.lastIdPcbAssing,block)
            #inserto las instruciones en la posicion indicada
            self.saveInstruction(block, instBlock)
            #actualizo estado de tabla
            self.refreshPositionInTable(pcb.getID(),block,1)
    ...
    def fetch(self, pcb, instruction):
        ...
        if pcb.hasPageInTable(page):
            ...
            if (not pcb.inHardDisk(page)):
                ...
                #self.lastIdPcbAssing = pcb.getID()
            ...
            #ir a disco,cargar la pagina a memoria,actualizar pcb saliente
